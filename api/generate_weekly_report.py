...
def group_users_by_id():
    ...
    for filename in os.listdir(INPUT_DIR):
        ...
        habit_info = {
            ...
            "habit_log": extract_last_7_days_logs(habit_data["habit_log"])
        }
        users[user_id]["habits"].append(habit_info)
    
    return users

def build_weekly_prompt(user_info, habits_data):
    ...
    return f"""
당신은 사용자 맞춤형 습관 코치입니다. 아래는 {nickname}의 최근 7일간의 모든 습관 기록입니다.
이 데이터를 바탕으로 다음 내용을 포함한 **친근하고 따뜻한 말투**로 종합 주간 리포트를 작성해주세요:

**[요청 항목]**
1. {nickname}의 이번 주 전체 습관 수행 요약 (전체 성공률: {overall_success_rate:.1f}%)
2. 각 습관별 성공률과 주요 성과 분석
3. 자주 실패한 원인과 습관 간 상관관계 분석
4. 요일별 전체 습관 성공/실패 패턴
5. 공감과 위로의 메시지
6. 다음 주를 위한 현실적이고 응원하는 제안
...
def generate_weekly_report_for_user(user_data):
    """API 호출을 위한 주간 리포트 생성 함수"""
    try:
        # 사용자 데이터에서 습관 정보 추출
        user_info = {
        ...
        # 최근 7일간 기록이 있는 습관만 필터링
        active_habits = [habit for habit in habits_data if habit.get("habit_log")]
        
        if active_habits:
            weekly_prompt = build_weekly_prompt(user_info, active_habits)
            return call_gemini(weekly_prompt)
        else:
            return f"{user_info['nickname']}님의 최근 7일간 습관 기록이 없어 주간 리포트를 생성할 수 없습니다."
    except Exception as e:
        return f"주간 리포트 생성 중 오류가 발생했습니다: {str(e)}"
...
def main():
    ...
    for user_id, user_data in users.items():
        ...
        if active_habits:
            weekly_prompt = build_weekly_prompt(user_data["user_info"], active_habits)
            nickname = user_data["user_info"]["nickname"]
            print(f"📡 {nickname}의 종합 주간 리포트 LLM 호출 중...")
            weekly_response = call_gemini(weekly_prompt)
            
            weekly_output_path = os.path.join(WEEKLY_OUTPUT_DIR, f"user_{user_id}_{nickname}_weekly_report.md")
            with open(weekly_output_path, "w", encoding="utf-8") as f:
                f.write(weekly_response)
            print(f"✅ {nickname}의 종합 주간 리포트 저장 완료: {weekly_output_path}")
        else:
            nickname = user_data["user_info"]["nickname"]
            print(f"⚠️ {nickname}: 최근 7일간 기록 없음 (주간 리포트 건너뜀)")
